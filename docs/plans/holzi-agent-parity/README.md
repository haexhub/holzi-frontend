...
Next up: [32-A — Agent-Self-Inventory + Self-Provisioning](./32a-agent-self-inventory.md) (finishes the Skills & Tools trio — meta-tools `list_tools` / `mcp_status` / `mcp_install` / `mcp_restart` on top of the Plan 32 CRUD, gated by Plan 21 approvals), or [29-B — Persona pro `agent_task`](./29b-persona-per-task.md), or [29-C — Persona pro Conversation](./29c-persona-per-conversation.md), or [17 — Cline/Roo first-class channel](./17-cline-roo-first-class-channel.md), or the remaining Plan 20 follow-up (README quickstart + troubleshooting + provider-setup docs), or another parity-push plan ([23](./23-composer-chips.md) / [26](./26-conversation-deep-links.md)), or [28 — Signal websocket receive](./28-signal-websocket-receive.md). Plan 18 (Profiles/Spaces) is "do not implement unless a new need appears" per the recommended order; Plan 19 is the bigger production-hardening pass.
...
- [32-A — Agent-Self-Inventory + Self-Provisioning](./32a-agent-self-inventory.md) —
  ...
  Granularität für `mcp_install`). **Start hier.**
