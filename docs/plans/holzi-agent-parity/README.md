...
Next up: [17 — Cline/Roo first-class channel](./17-cline-roo-first-class-channel.md), or the remaining Plan 20 follow-up (README quickstart + troubleshooting + provider-setup docs), or another parity-push plan ([23](./23-composer-chips.md) / [24](./24-workspace-git-extended.md) / [26](./26-conversation-deep-links.md) / [27](./27-insights-and-logs.md)). Plan 18 (Profiles/Spaces) is "do not implement unless a new need appears" per the recommended order; Plan 19 is the bigger production-hardening pass.
