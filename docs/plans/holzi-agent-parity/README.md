...
Next up: [17 — Cline/Roo first-class channel](./17-cline-roo-first-class-channel.md), or one of the remaining two Plan 20 follow-ups (onboarding empty state, README/troubleshooting/provider docs), or "Plan 20-B" (dev-stack docker-agnostisch) — the local stack default is podman, but the user's host is docker-only so `make up-local-full` doesn't run. Plan 18 (Profiles/Spaces) is "do not implement unless a new need appears" per the recommended order; Plan 19 is the bigger production-hardening pass.
