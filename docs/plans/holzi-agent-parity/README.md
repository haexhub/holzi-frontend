...
Next up: [32 — MCP-Server CRUD](./32-mcp-server-crud.md) (next slice of the Skills & Tools trio — activates the disabled „MCP-Server konfigurieren"-Sprungpunkt that Plan 31 wired and unlocks Plan 32-A's self-provisioning), or [29-B — Persona pro `agent_task`](./29b-persona-per-task.md), or [29-C — Persona pro Conversation](./29c-persona-per-conversation.md), or [17 — Cline/Roo first-class channel](./17-cline-roo-first-class-channel.md), or the remaining Plan 20 follow-up (README quickstart + troubleshooting + provider-setup docs), or another parity-push plan ([23](./23-composer-chips.md) / [26](./26-conversation-deep-links.md)), or [28 — Signal websocket receive](./28-signal-websocket-receive.md). Plan 18 (Profiles/Spaces) is "do not implement unless a new need appears" per the recommended order; Plan 19 is the bigger production-hardening pass.
...
- [32 — MCP-Server CRUD](./32-mcp-server-crud.md) — externe MCP-Server
  ...
  Konfigurieren-Sprungpunkte aus Plan 31. **Start hier.**
