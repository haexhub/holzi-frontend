...
Next up: [29-B — Persona pro `agent_task`](./29b-persona-per-task.md), or [29-C — Persona pro Conversation](./29c-persona-per-conversation.md), or [17 — Cline/Roo first-class channel](./17-cline-roo-first-class-channel.md), or the remaining Plan 20 follow-up (README quickstart + troubleshooting + provider-setup docs), or another parity-push plan ([23](./23-composer-chips.md) / [26](./26-conversation-deep-links.md)), or [28 — Signal websocket receive](./28-signal-websocket-receive.md). The Skills & Tools trio (31 / 32 / 32-A) is complete; a known Plan-32 follow-up remains optional: remount-free live `/mcp` is done, but [29-E — Tools/MCPs pro Persona](./29e-persona-tools-and-mcps.md) is the natural next consumer of the meta-tools. Plan 18 (Profiles/Spaces) is "do not implement unless a new need appears" per the recommended order; Plan 19 is the bigger production-hardening pass.
...
**As of 2026-06-03: 21, 22, 24, 25, 27 are merged (see Completed above); only 23 and 26 remain open in this push.**
